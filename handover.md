...
#### 1. ⚠️ IndexNow 워크플로는 존재하지 않았다 — 이번 세션에서 새로 만들었다

사용자는 이미 설정해뒀다고 했으나, **저장소에 `.github` 디렉터리 자체가 없었고** GitHub API로 확인한 워크플로는 기본 `pages-build-deployment` 하나뿐이었다. 처음부터 구축했다.
...
**🔴 다음 세션이 반드시 확인할 것**
- **신규 2개 페이지는 아직 IndexNow에 제출되지 않았다.** 이 페이지들을 담은 커밋(`9b45059`)의 IndexNow 실행이 시크릿 부재로 실패했고, 이후 수정 커밋들은 경로 필터에 안 걸려 트리거되지 않았다.
- **샌드박스에서 검증 못 한 것**: `petpawcalc.com`이 네트워크 allowlist 밖이라 키 파일 접근성과 실제 POST를 확인하지 못했고, 토큰에 `actions:write`가 없어 수동 dispatch도 못 했다. **키 해석 로직·URL 생성·엣지케이스는 전부 로컬에서 테스트했지만, 엔드투엔드는 미검증이다.**
- **조치**: 사용자가 GitHub Actions 탭 → "IndexNow submit" → Run workflow를 한 번 누르면 96개 전체가 제출된다. 또는 다음 콘텐츠 푸시에서 자동으로 돈다. 다음 세션에서 실행 로그를 반드시 확인할 것.
