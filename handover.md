...
- **애드센스 재검토 요청 결과 확인.** 구글 재크롤에 며칠~1주 필요하므로 배포 직후가 아니라 시간을 두고 요청하도록 안내했다.
- **거절 시**: 같은 사유면 남은 후보는 (a) 800단어 미만 7개 추가 보강, (b) 툴 페이지 템플릿 동일성 완화, (c) 페이지 수 축소·통합. **키워드 스터핑은 이미 제거했으므로 그 축으로는 더 할 게 없다.**
